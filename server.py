...
# Copyright (C) 2026 Jared Rhodenizer (original barehands)
...
def public_config():
    """What the page sees of barehands.json: the ring's name + orb list."""
    return {"name": CONFIG.get("name", "Assistant"),
...
def notes_tree(idx):
    """A notes orb's .md tree as (json, status). Shared with build_static."""
    root = orb_root(idx)
    if root is None or not root.is_dir():
        return {"name": "?", "notes": [], "dirs": []}, 404
...
def props_tree():
    """The media airlock as a browsable tree, as (json, status)."""
    ...
    try:
        ...
        return {"name": "Props", "items": [], "dirs": []}, 500
